...
inner join safra s on s.cod_produto = p.cod_produto where p.cod_produto = 1;

-- • Calcular o volume total de safras para cada produtoCalcular o volume total de safras para cada produto
select p.nome_produto, s.volume, s.ano, sum(s.volume) as volume_total from produtos p 
inner join safra s on s.cod_produto = p.cod_produto group by p.nome_produto;

-- • Listar os produtos que tiveram safras no ano de 2023
select p.nome_produto, s.ano from produtos p
inner join safra s on s.cod_produto = p.cod_produto where s.ano = 2023;

-- • Listar o nome do produto e o volume médio das safras para produtos com volume médio superior a 100 milhões de toneladas
select p.nome_produto, s.ano, avg(s.volume) as volume_medio from produtos p
inner join safra s on s.cod_produto = p.cod_produto group by p.nome_produto having avg(s.volume) > 100000000;

-- • Calcular o total de volume de safras agrupados por produtos no ano de 2023
select p.nome_produto, s.volume, s.ano, sum(s.volume) as volume_total from produtos p
inner join safra s on s.cod_produto = p.cod_produto where s.ano = 2023 group by p.nome_produto;

-- • Calcular volume de safras do produto com código 2 no ano de 2023 no mês de Junho
select sum(s.volume), s.cod_produto from produtos p
inner join safra s on s.cod_produto = p.cod_produto where s.cod_produto = 2 and s.ano = 2023 and s.mes = 6;
